...
capture_images = [
    imageHelpers.ImageFile(
        image_file, os.path.join(image_location, image_file)
    )
    for image_file in os.listdir(image_location)
    if os.path.isfile(os.path.join(image_location, image_file))
    and capture_re.match(image_file)
]
max_x = 0
max_y = 0
z_levels = []
...
for capture in capture_images:
    ...
    if capture.cords[2] not in z_levels:
...
for z in z_levels:
    print(f"Merging map from {str(z)} z level.")
